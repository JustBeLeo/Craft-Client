...
public class HtmlUtils {
    // css样式，隐藏header
    private static final String HIDE_HEADER_STYLE = "<style>" +
            "div.headline{display:none;}%s" +
            "</style>";
    // css style tag, 需要格式化
    ...
    public static String createCssTag(String url) {
        return String.format(HIDE_HEADER_STYLE, url);
    }
    ...
    public static String createHtmlFromObjects(List<Object> list) {
        StringBuffer sb = new StringBuffer();
        for (Object o : list) {
            if (o instanceof EditText) {
                sb.append("<p>\n")
                        .append(((EditText) o).getText().toString()).append("\n")
                        .append("<p>\n");
            } else if (o instanceof String) {
                sb.append("<div>\n")
                        .append("<img src = ").append(o.toString()).append(" />\n")
                        .append("</div>");
            }
        }
        return sb.toString();
