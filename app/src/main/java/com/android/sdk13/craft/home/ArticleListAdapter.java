...
import com.android.sdk13.craft.entity.HomeRecommendItem;
...
public class ArticleListAdapter extends BaseRecycleAdapter<Article> {

    public ArticleListAdapter(List<Article> list, Context mContext, RecyclerView recyclerView) {
    ...
    public void onBindViewHolder(@NonNull BaseRecycleViewHolder holder, int i) {
        if (holder instanceof myHolder) {
            ...
            ((myHolder) holder).tvTitle.setText( item.getTitle() );
            ((myHolder) holder).tvType.setText( item.getType().getName() );
            ((myHolder) holder).tvDetail.setText( item.getText() );
